...
import { ISellerGig } from '@taylordurden/jobber-shared';

const log = getLogger('authElasticSearchServer', 'debug');
...
async function createIndex(indexName: string): Promise<void> {
  try {
    const existed: boolean = await checkIfIndexExists(indexName);
    if (existed) {
